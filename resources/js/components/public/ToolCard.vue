...
<template>
    ...
    >
        <div v-if="tool.is_sponsored" class="absolute top-3 right-3 z-10">
            <Badge
                class="gap-1 bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300"
            >
                <Star class="size-3" />
                Sponsorisé
            </Badge>
        ...
        <div class="flex flex-col gap-4 p-5">
            <div class="flex items-start gap-4">
                <div
                    class="flex size-12 shrink-0 items-center justify-center overflow-hidden rounded-lg border border-border bg-muted/50"
                >
                    <img
                        :src="tool.logo_url"
                        :alt="`Logo ${tool.name}`"
