...
const selectedToolA = computed(() =>
    props.tools.find((t) => t.id === form.tool_a_id),
);
const selectedToolB = computed(() =>
    props.tools.find((t) => t.id === form.tool_b_id),
);
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="mx-auto max-w-2xl p-4">
            <Heading
                ...
                description="Comparer deux outils de développement"
            />
            ...
            <form class="space-y-6" @submit.prevent="submit">
                <div class="grid gap-2">
                    ...
                    >
                        <option value="" disabled>
                            Sélectionner l'outil A
                        </option>
                        <option
                            v-for="tool in filteredToolsA"
                            :key="tool.id"
                            :value="tool.id"
                        >
                            {{ tool.name }}
                ...
                <div class="grid gap-2">
                    ...
                    >
                        <option value="" disabled>
                            Sélectionner l'outil B
                        </option>
                        <option
                            v-for="tool in filteredToolsB"
                            :key="tool.id"
                            :value="tool.id"
                        >
                            {{ tool.name }}
